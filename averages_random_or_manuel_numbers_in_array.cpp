...
int main()
{
	...
	if(((int)choice==77)||((int)choice==109)||((int)choice==82)||((int)choice==114))			
	{
		...
		for(i=0;i<arraySize;i++)
		{
			...
			// standard deviation
			sumOfSquares+=pow(array_numbers[i], 2);
			squaresOfSum+=pow((sumOfOdd + sumOfEven), 2);			
		}		
		cout << endl;
		...
		standardDeviation = ((arraySize*(sumOfSquares)) - (pow(squaresOfSum, 2))) / (arraySize*(arraySize-1));
		standardDeviation = pow(standardDeviation, (float)1/2);
